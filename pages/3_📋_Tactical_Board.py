...
# --- 2. HTML OLUŞTURUCU (Garantili Yöntem) ---
def generate_board_html(formation_name):
    players = FORMATIONS.get(formation_name, FORMATIONS["4-3-3"])
    players_html = ""
    for p in players:
        # Python değişkenleri tek parantez { }, CSS/JS çift parantez {{ }}
        players_html += f'<div class="player-dot" style="left: {p["x"]}%; top: {p["y"]}%;">{p["l"]}</div>\n'

    # DİKKAT: CSS içindeki tüm { işaretleri {{ olarak, } işaretleri }} olarak değiştirildi.
    # Python bu sayede hata vermez.
    html_code = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
